...
2. **Profiles are data; executable-local code lives with the executable.** `profile.json` is pure JSON + `prompt.md` is markdown. **Executable-local TypeScript and shell scripts are encouraged inside `src/executables/<name>/`** — any logic that only this executable uses belongs with it (e.g. `src/executables/resolve/apply-prefer.sh`, `src/executables/release/changelog.ts`). `src/scripts/` is reserved for cross-cutting utilities used by multiple executables (`commitAndPush`, `composePrompt`, `verify`, `ensurePr`, `postIssueComment`). **Prefer a small shell script colocated with the executable over bloating `src/scripts/` with profile-specific logic.** Adding a new command = drop a new `src/executables/<name>/` dir with its profile + prompt + any executable-local scripts; register any new *shared* scripts. Dispatch is profile-driven — no edits needed for issue- or PR-triggered commands.
