...
beforeEach(() => {
  ...
  mocks.files.clear()
  mocks.files.set(`${CWD}/.kody-engine/definitions`, "")
...
describe("loadAgentAdhoc: agent resolution", () => {
  it("reads agent, extracts H1 title + body, uses message-arg fallback", async () => {
    mocks.files.set(`${CWD}/.kody-engine/definitions/agents/dr-bug.md`, "# Dr. Bug\n\nI hunt regressions.")
  ...
  it("strips frontmatter before parsing the H1 title", async () => {
    mocks.files.set(`${CWD}/.kody-engine/definitions/agents/qa.md`, "---\nagent: qa\n---\n# QA Lead\n\nBody here.")
  ...
  it("humanizes the slug when the agent has no H1", async () => {
    mocks.files.set(`${CWD}/.kody-engine/definitions/agents/lead_dev.md`, "Just prose, no heading.")
  ...
  it("defaults thread to empty string when absent", async () => {
    mocks.files.set(`${CWD}/.kody-engine/definitions/agents/x.md`, "# X")
