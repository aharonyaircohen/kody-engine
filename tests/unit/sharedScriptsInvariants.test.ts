...
const CONSUMER_LIBRARY_SCRIPTS: Record<string, string> = {
  ...
  loadCompanyIntents: "agency-architect",
  loadCompanyPortfolio: "agency-architect",
  loadJobFromFile: "capability-tick",
  loadAgentAdhoc: "agent-ask",
  markFlowSuccess: "revert",
  mergeFlow: "merge",
  parseAgencyArchitectDecision: "agency-architect",
...
describe("script catalog: removed executable-era names", () => {
  it("does not keep writeRunSummary as a script alias", async () => {
    ...
    expect(scripts.allScriptNames.has("writeRunSummary")).toBe(false)
  })
})
