...
const KNOWN_SOLO_SCRIPTS: Record<string, SoloEntry> = {
  ...
  openAgentFactoryStatePr: {
    owner: "agent-factory",
    reason: "agent-factory-only state-repo branch and PR creator for reviewed model definitions.",
  },
  ...
  recordOutcome: { owner: "task-job-fail-once", reason: "task-job-fail-once-only outcome recorder." },
  failOnceTaskJob: { owner: "task-job-fail-once", reason: "task-job-fail-once-only failure-once task harness." },
  planTaskJobs: { owner: "task-jobs", reason: "task-jobs-only task plan builder." },
  dispatchNextTaskJob: { owner: "task-jobs", reason: "task-jobs-only next-task dispatcher." },
