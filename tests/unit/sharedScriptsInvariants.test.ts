...
const KNOWN_SOLO_SCRIPTS: Record<string, SoloEntry> = {
  ...
  loadIssueContext: { owner: "agent-factory", reason: "agent-factory-only issue request loader for model PRs." },
  ...
  applyCapabilityReports: { owner: "ci-check", reason: "ci-check-only capability report emitter." },
  markFlowSuccess: { owner: "merge", reason: "merge-only terminal status setter." },
  ...
  resolveFlow: { owner: "resolve", reason: "resolve-only flow step." },
