...
import { writeAgentRunSummary } from "../../src/scripts/writeAgentRunSummary.js"
...
describe("writeAgentRunSummary", () => {
  ...
  it("writes a success summary with PR url", async () => {
    ...
    await writeAgentRunSummary(ctx, fakeProfile("run"), null)
    const written = fs.readFileSync(summaryFile, "utf-8")
    expect(written).toMatch(/success/)
    expect(written).toMatch(/\*\*Implementation:\*\* `run`/)
  ...
  it("labels exit 3 as no-op", async () => {
    ...
    await writeAgentRunSummary(ctx, fakeProfile("resolve"), null)
    const written = fs.readFileSync(summaryFile, "utf-8")
  ...
  it("silently no-ops when GITHUB_STEP_SUMMARY is not set", async () => {
    ...
    const ctx = baseCtx()
    await expect(writeAgentRunSummary(ctx, fakeProfile("run"), null)).resolves.toBeUndefined()
