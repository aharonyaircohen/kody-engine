 ...
 * test mirrors the resolution rule (perExecutable → profile.model → agent.model)
...
describe("model resolution precedence (mirror)", () => {
  ...
  it("perExecutable wins over agent.model", () => {
    const cfg: KodyConfig = {
      ...baseConfig,
      agent: {
        model: "claude/claude-sonnet-4-6",
        perExecutable: { classify: "claude/claude-haiku-4-5-20251001" },
      },
    }
    expect(resolveModel(cfg, "classify", "inherit")).toBe("claude/claude-haiku-4-5-20251001")
  })

  it("perExecutable wins even when the profile names an explicit model", () => {
    const cfg: KodyConfig = {
      ...baseConfig,
      agent: {
        model: "claude/claude-sonnet-4-6",
        perExecutable: { plan: "claude/claude-haiku-4-5-20251001" },
      },
    }
    expect(resolveModel(cfg, "plan", "claude/claude-opus-4-7")).toBe("claude/claude-haiku-4-5-20251001")
  })

  it("missing perExecutable entry falls through to the normal precedence", () => {
    const cfg: KodyConfig = {
      ...baseConfig,
      agent: {
        model: "claude/claude-sonnet-4-6",
