...
describe("Engine release workflow", () => {
  it("publishes only through the Kody workflow", () => {
    ...
    expect(kody).toContain('if [[ "$KODY_CAPABILITY" == "npm-publish" ]]')
    expect(kody).toContain("node dist/bin/kody.js")
    expect(kody).toContain("npx -y -p @kody-ade/kody-engine@latest kody-engine")
