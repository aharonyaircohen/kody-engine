...
describe("runJob (Phase 1 seam)", () => {
  ...
  it("still notifies completion when the terminal Workflow state cannot be saved", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kody-workflow-terminal-save-"))
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true)
    try {
      writeSimpleCapability(cwd, "check")
      writeWorkflowDefinition(cwd, "quality-run", {
        name: "Quality Run",
        agent: "kody",
        steps: [{ capability: "check" }],
      })
      hasGitHubActionsIdentity.mockReturnValue(true)
      hasStateBackendConfig.mockReturnValue(true)
      stateBackend.saveWorkflowRun
        .mockResolvedValueOnce(undefined)
        .mockRejectedValue(new Error("Kody backend request failed (500)"))
      runImplementationChain.mockResolvedValueOnce({
        exitCode: 0,
        usage: {
          version: 1,
        ...
        capabilityOutput: { status: "pass" },
        capabilityResults: [capabilityResult({ status: "pass" })],
      })

      const result = await runJob(
        {
          workflow: "quality-run",
          workflowRunId: "run-quality-1",
          cliArgs: {},
          flavor: "instant",
        },
        {
        ...
        expect.objectContaining({
          ...
          output: expect.objectContaining({
            usage: expect.objectContaining({
              tokens: expect.objectContaining({ total: 150 }),
            }),
          }),
        }),
      )
      expect(stderr).toHaveBeenCalledWith(expect.stringContaining("failed to save terminal Workflow state"))
    } finally {
      stderr.mockRestore()
      fs.rmSync(cwd, { recursive: true, force: true })
    }
