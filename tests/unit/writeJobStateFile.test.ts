 ...
 * capability's next state via the configured `JobStateBackend`.
 *
 * Focus: the parse-error branch (the capability failed to emit a
 * `kody-job-next-state` block but the run must not wedge the capability forever).
 * Without this branch, a flaky model that drops the fenced block would
 * leave the capability stuck "overdue" on the dashboard, re-firing every cron
...
describe("writeJobStateFile: parse-error path", () => {
  it("carries the prior state forward (cursor + data unchanged) so the capability does not wedge", async () => {
    // Prior state at rev 3 with cursor "step-2" and the capability's per-item ledger
    ...
    // cursor + ledger carry forward — the capability is still at step-2 next time.
  ...
  it("records the failure on a first-ever tick with no prior state (no wedge, no carry)", async () => {
    ...
    // the run fails loudly — without writing a state file, the capability will
    // retry next wake as before (legacy behavior).
    const slug = "fresh-capability"
    const ctx = ctxFor(slug, {
      nextStateParseError: "missing `kody-job-next-state` block in agent output",
