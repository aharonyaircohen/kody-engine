 ...
 * agentResponsibility's next state via the configured `JobStateBackend`.
 *
 * Focus: the parse-error branch (the agentResponsibility failed to emit a
 * `kody-job-next-state` block but the run must not wedge the agentResponsibility forever).
 * Without this branch, a flaky model that drops the fenced block would
 * leave the agentResponsibility stuck "overdue" on the dashboard, re-firing every cron
...
function loadedFor(slug: string, state: StateEnvelope): LoadedJobState {
  return {
    path: `.kody/agent-responsibilities/${slug}/state.json`,
    handle: null,
    state,
    created: false,
  }
...
describe("writeJobStateFile: parse-error path", () => {
  it("carries the prior state forward (cursor + data unchanged) so the agentResponsibility does not wedge", async () => {
    // Prior state at rev 3 with cursor "step-2" and the agentResponsibility's per-item ledger
    ...
    const stateDir = path.join(tmp, ".kody", "agent-responsibilities", slug)
    ...
    await writeJobStateFile(ctx, PROFILE, null, { jobsDir: ".kody/agent-responsibilities" })

    const after = JSON.parse(
      fs.readFileSync(path.join(tmp, ".kody", "agent-responsibilities", slug, "state.json"), "utf-8"),
    ) as StateEnvelope
    // rev was bumped so the next tick sees a fresh state.
    expect(after.rev).toBe(4)
    // cursor + ledger carry forward — the agentResponsibility is still at step-2 next time.
  ...
  it("records the failure on a first-ever tick with no prior state (no wedge, no carry)", async () => {
    ...
    // the run fails loudly — without writing a state file, the agentResponsibility will
    // retry next wake as before (legacy behavior).
    const slug = "fresh-agentResponsibility"
    const ctx = ctxFor(slug, {
      nextStateParseError: "missing `kody-job-next-state` block in agent output",
    ...
    expect(fs.existsSync(path.join(tmp, ".kody", "agent-responsibilities", slug, "state.json"))).toBe(false)
  })
})
