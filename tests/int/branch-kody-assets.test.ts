 ...
 * (engine v0.4.196–198). Consumer repos track their agentActions at
 * `.kody/agent-actions/<name>/` but `.gitignore` ignores `.kody/*` and
 * re-includes them via a negation (`!.kody/agent-actions/**`). On an ephemeral
 * CI runner, branch setup's `git clean -fd` directory-walk over that
 * negated-ignore pattern could nuke the whole `.kody/agent-actions/<name>`
...
/**
 * A consumer repo with a tracked agentAction under the negated-ignore path —
...
describe("integration: branch setup preserves tracked .kody assets", () => {
  ...
  it("keeps .kody/agent-actions/<name> intact when creating a feature branch", () => {
    const promptPath = path.join(repo.workdir, ".kody", "agent-actions", "run", "prompt.md")
    ...
    // The load-bearing assertion: the tracked agentAction survived branch setup.
    expect(fs.existsSync(promptPath)).toBe(true)
    expect(fs.readFileSync(promptPath, "utf-8")).toBe("do the thing\n")
    expect(fs.existsSync(path.join(repo.workdir, ".kody", "agent-actions", "run", "profile.json"))).toBe(true)
  ...
  it("restores .kody assets even after they are removed from the working tree", () => {
    ...
    // Reproduce the worst case directly: the agentAction dir is gone from the
    ...
    fs.rmSync(path.join(repo.workdir, ".kody", "agent-actions"), { recursive: true, force: true })
