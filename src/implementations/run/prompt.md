...
1. **Research** — read the issue carefully, then meet the research floor below before any Edit/Write. Use Grep/Glob/Read to investigate.
   ...
   **Research floor (MUST be met before step 3):**
   ...
   - A failing path may be generated, aliased, or stale. If it does not exist, locate the real tracked source with Glob/Grep or `git ls-files` before editing. Never recreate an existing module under a guessed directory. New files are allowed only when the task actually requires a new module, and their parent directory must already exist or be created explicitly.
