...
export const DUTY_PROFILE_FILE = "profile.json"
export const DUTY_BODY_FILE = "duty.md"

export interface DutyFolderConfig {
  action?: string
  executable?: string
  every?: ScheduleEvery
  tickScript?: string
...
export interface DutyFolder {
  ...
  body: string
  rawBody: string
  config: DutyFolderConfig
  rawProfile: Record<string, unknown>
}

export function listDutyFolderSlugs(absDir: string): string[] {
  ...
  return entries
    ...
    .filter((e) => isDutyFolder(path.join(absDir, e.name)))
    .map((e) => e.name)
    .sort()
}

export function isDutyFolder(dir: string): boolean {
  return fs.existsSync(path.join(dir, DUTY_PROFILE_FILE)) && fs.existsSync(path.join(dir, DUTY_BODY_FILE))
}

export function readDutyFolder(root: string, slug: string): DutyFolder | null {
  const dir = path.join(root, slug)
  const profilePath = path.join(dir, DUTY_PROFILE_FILE)
  const bodyPath = path.join(dir, DUTY_BODY_FILE)
...
export function parseDutyConfig(raw: Record<string, unknown>): DutyFolderConfig {
...
export function parseDutyBody(raw: string, slug: string): { title: string; body: string } {
