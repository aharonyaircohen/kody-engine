/**
 * AgentResponsibility fan-out fallback tick (runs inside pool-serve on the always-on machine).
 *
 * GitHub Actions' cron normally fires the scheduled agentResponsibility/goal fan-out every
 * 15 min. When GitHub Actions is DOWN that cron never fires, so agentResponsibilities stall.
 ...
 * per-agentResponsibility cadence guard (`lastFiredAt` vs `every:`) means that even if GitHub
 * recovers mid-window and both fire, a agentResponsibility won't run twice.
 *
 * Kept dependency-injected (no direct PoolRegistry / fetch import) so it's
 * unit-testable without network or a live pool.
 */

export interface AgentResponsibilityTickClaimResult {
  ok: boolean
  machineId?: string
  reason?: string
}

export interface AgentResponsibilityFallbackDeps {
  ...
  ) => Promise<AgentResponsibilityTickClaimResult>
  log: (msg: string) => void
  /** Injectable clock for deterministic jobIds in tests. */
  now?: () => number
}

export interface AgentResponsibilityTickResult {
