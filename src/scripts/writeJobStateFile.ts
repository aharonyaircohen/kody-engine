 ...
 * "failed this tick, retry next cadence" instead of wedging the capability forever
...
export const writeJobStateFile: PostflightScript = async (ctx, _profile, agentResult, args) => {
  ...
  if (parseError) {
    ...
    // The tick failed to propose a next state, but it must not wedge the capability
    ...
    // failure. The capability then retries on its next cadence instead of re-firing
  ...
  // Stamp `lastFiredAt` so dispatchCapabilityFileTicks can gate per-capability
  ...
  // result, so the dashboard can show "last run failed / took 2m" per capability
