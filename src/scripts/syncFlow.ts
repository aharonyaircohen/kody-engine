 ...
 *     implementation's profile, where syncFlow IS the run. Leave unset (false)
 *     when used as a preflight in another implementation — the parent owns the
 *     user voice on success.
 *
 * Failure paths always announce — the user needs to know why their run
 * stopped, regardless of which implementation triggered it.
 *
...
/**
 * Re-stamp kody:done on the PR. The sync implementation's preflight stamps
