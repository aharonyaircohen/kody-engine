 ...
 * (kept that way to avoid breaking every existing capability in the wild). The
 * `kody-capability-next-state` label is the new wording introduced alongside the
 * capability-pipeline rename; if the configured label yields no block, the
...
const CAPABILITY_NEXT_STATE_FENCE_ALIASES: Record<string, string> = {
...
export const parseJobStateFromAgentResult: PostflightScript = async (ctx, _profile, agentResult, args) => {
  ...
  // Preferred path: the agent called the `submit_state` tool (capability-tick with
  ...
  if (result.error) {
    ...
    // Evergreen capabilities (approval-gate, qa) routinely check their queue, find
    ...
    // instead of being flagged "Capability failed". A genuinely cut-off run
