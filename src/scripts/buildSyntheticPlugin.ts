 ...
 *   1. The agentAction's own directory:
 *      the agentAction directory `{skills,commands,agents,hooks}/<entry>`
 *      — for parts that are specific to one agentAction.
 *   2. The engine's shared catalog:
 *      src/plugins/{skills,commands,agents,hooks}/<entry>
 *      — for parts reused across multiple agentActions.
 */
...
export const buildSyntheticPlugin: PreflightScript = async (ctx, profile) => {
  ...
  // Resolve plugin parts from the agentAction's own directory first, then fall
  // back to the engine's central catalog. Lets an agentAction ship local
  // skills/commands/subagents/hooks under
  //   the agentAction directory `{skills,commands,agents,hooks}/
