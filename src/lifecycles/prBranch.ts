 ...
 * canonical chain that PR-branch implementations run.
 *
 ...
 * - `sync`: not all PR-branch implementations fast-forward the local branch
 ...
 * - `finalize`: single-session implementations (feature, bug — no orchestrator)
...
import type { Profile, ScriptEntry } from "../implementations/types.js"
...
export function prBranchLifecycle(profile: Profile, profilePath: string): void {
  ...
  // Single-session implementations (no orchestrator) stamp their own terminal
