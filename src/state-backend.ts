...
export interface LoopDispatchDecision {
  kind: "fire"
  reason: string
  scheduledAt?: string
}

function serializeLoopDispatchDecision(decision: LoopDispatchDecision): LoopDispatchDecision {
  return {
    kind: decision.kind,
...
export interface LoopDispatchReservation {
  idempotencyKey: string
  loopId: string
  decision: LoopDispatchDecision
  leaseUntil: string
...
export interface StateBackend {
  ...
  saveAgentState(tenantId: string, state: unknown, expectedRevision?: number): Promise<void>
  resetAgentState(tenantId: string, agent: string): Promise<void>
  ...
  reserveLoopDispatch(
    tenantId: string,
    reservation: LoopDispatchReservation,
  ): Promise<{
    acquired: boolean
    dispatchId?: string
  ...
  finishAgencyRun(tenantId: string, run: unknown, now: string): Promise<void>
  appendRunEvent(
    tenantId: string,
    runId: string,
...
export function createStateBackendFromEnv(
  ...
  return {
    ...
    async getAgentState(tenantId, agent) {
      const result = await transport.query(anyApi.agentStates.get, {
        tenantId: requireTenant(tenantId),
        agent: requireNonEmpty(agent, "agent"),
      })
      return (result as AgentStateDocument | null) ?? null
    },
    async saveAgentState(tenantId, state, expectedRevision) {
    ...
    async resetAgentState(tenantId, agent) {
    ...
    async reserveLoopDispatch(tenantId, reservation) {
    ...
    async createAgencyRun(tenantId, subjectType, subjectId, run, now) {
    ...
    async finishAgencyRun(tenantId, run, now) {
