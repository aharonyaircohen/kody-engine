...
function resetWorkingTree(cwd?: string): void {
  ...
  try {
    // Backend definitions are hydrated into an untracked engine-owned cache.
    // Preserve that cache across the branch reset performed within one run.
    execFileSync("git", ["clean", "-fd", "-e", ".kody-engine"], {
      cwd,
...
export function checkoutPrBranch(prNumber: number, cwd?: string): string {
  ...
  try {
    // Preserve the backend-hydrated definition cache during checkout.
    execFileSync("git", ["clean", "-fd", "-e", ".kody-engine"], {
      cwd,
      env,
      stdio: ["ignore", "pipe", "pipe"],
...
export function ensureFeatureBranch(
  ...
  return ensureFeatureBranchInner(issueNumber, title, defaultBranch, cwd, baseBranch)
}
