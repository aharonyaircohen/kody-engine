 ...
 */
export function getExecutableRoots(cwd: string = process.cwd()): string[] {
...
export function resolveCapabilityExecution(
  ...
  const cliArgs = executableDeclaresInput(executable, "capability", roots) ? { capability: capability.slug } : {}
  return { executable, cliArgs }
}

function executableDeclaresInput(
  executable: string,
  inputName: string,
  roots: string | string[] = getExecutableRoots(),
): boolean {
