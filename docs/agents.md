...
concrete scopes, cadence, tools, or output contracts. Those belong in
...
Stay inside the scope handed to you by the capability.
```
...
| What work does this agent own? | no | Broad capability only. Concrete work belongs in capabilities. |
...
5. Wire the agent through capabilities or executables with `agent`.
...
- Do not create agent for every capability if an existing agent fits.
- Do not let agent override safety rules from `AGENTS.md`, executable profiles, or capability restrictions.
