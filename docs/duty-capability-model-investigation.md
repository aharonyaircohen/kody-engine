# Duty Capability Model Investigation

## Verdict

The proposed Observe / Act / Verify model is a strong simplification if it is
treated as the public capability contract for a duty-like unit, not as a new
orchestration layer.
...
- A duty/capability does one kind of work and returns structured output.
...
Should Kody simplify duties into one reusable capability model with one of three
contract kinds?
...
Those remain separate. It asks whether duty/executable authoring becomes simpler
...
- Duty = why/when a standing responsibility exists.
- Executable = how work runs.
...
- A managed goal reads missing evidence, chooses a route step, dispatches a
  duty/executable, then waits for reported evidence.
- Duty reports can add facts/evidence to goal state, but cannot set goal state,
  route, destination, or completion directly.

That means the proposed simplification aligns with the direction of the system:
duties should produce facts, changes, or evidence, while goals and tasks decide
...
- `docs/jobs-model.md`: defines goal, duty, executable, job, and run, and
  states that a job points to one executable.
- `docs/duties.md`: defines duties as standing responsibility, intent,
  metadata, routing, cadence, and allowed outputs.
- `docs/executables.md`: defines executables as concrete runnable actions and
  documents duty reports.
- `docs/goals.md`: defines managed goals as outcome managers that route missing
  evidence to duties/executables.
...
- `src/goal/manager.ts`: plans one goal tick from missing evidence and route
  facts.
- `src/scripts/applyDutyReports.ts`: applies factual duty reports without
  letting reports own goal structure.
- `src/scripts/planTaskJobs.ts` and `src/scripts/dispatchNextTaskJob.ts`:
  keep multi-step task work in task/job machinery rather than inside one duty.
...
- `release`: this is a full process. It should become a goal route or task plan,
  not a single duty capability.
- `task-leader`: this sounds like a manager/orchestrator. It should not be a
  normal duty capability unless reduced to one clear output.
- `goal-manager`: this is intentionally above duties. It should not be
  reclassified as Observe, Act, or Verify.
- `duty-scheduler` and `goal-scheduler`: these are engine scheduling helpers,
...
- Duty writes long-term progress directly.
- Duty mutates goal route/state/destination.
- Duty decides the whole business outcome is done.
...
side-effects, the duty is too large.
...
The biggest risk is moving orchestration out of "duty" in name only, then hiding
...
duty/profile/executable system. The user-facing model can simplify before the
...
Add optional `capabilityKind` to profiles or duty profiles:
...
Start with duty reports and task outputs:
...
Create a catalog report that lists every duty/executable and one of:
...
- Goal reports cannot write `state`, `route`, `destination`, or `duties`.
...
4. Confirm the remodeled flow does not need duty-owned progress state.
...
single duty owning the whole process.

Reject or revise it if too many real duties require this shape:

```text
inspect -> decide -> act -> verify -> update long-term progress
```

That shape is not a duty. It is a manager loop, and Kody already has a better
