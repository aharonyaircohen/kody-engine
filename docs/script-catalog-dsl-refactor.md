...
Stop `src/scripts/` from being a junk drawer of single-agentAction orchestration code, by making the `profile.json` DSL expressive enough that custom `xxxFlow.ts` glue files become unnecessary.

**Success criteria:**
1. Every script in `src/scripts/` is referenced by ≥2 agentActions (mechanically enforced).
2. The 5 PR-branch agentActions (`fix`, `revert`, `resolve`, `fix-ci`, `init`) all run via a single shared lifecycle, with no per-agentAction `xxxFlow.ts` file.
...
- 69 distinct scripts referenced across 29 profiles (`src/agent-actions/*/profile.json`).
- **35 are referenced by exactly 1 agentAction** — violating the invariant in [CLAUDE.md](../CLAUDE.md) that `src/scripts/` is "only cross-cutting utilities used by multiple agentActions."
- 8 are used by 2 agentActions.
- 26 are used by 3+ agentActions (genuine shared utilities).
...
| Dispatch/classify | `classifyByLabel`, `dispatchClassified`, `recordClassification`, `dispatch`, `dispatchAgentResponsibilityFileTicks` | `classify`, `spec`, `agent-responsibility-scheduler` |
| AgentResponsibility tick | `loadJobFromFile`, `parseJobStateFromAgentResult`, `runTickScript` | `agent-responsibility-tick`, `agent-responsibility-tick-scripted` |
...
The naming tell is `xxxFlow` — these are per-agentAction orchestration scripts pretending to be shared utilities.

## Why this happened

The profile DSL today only offers low-level primitives (run-script-X, then run-script-Y, with optional `runWhen`). When an agentAction needs orchestration logic that can't be expressed as a flat list of scripts (branching, retries, state-load-fanout), it has two routes:

1. Inline TS inside the agentAction dir — **forbidden** by invariant 2.
...
`lifecycle: "pr-branch"` is a macro. At profile-load time (in `src/profile.ts`), it expands to the canonical preflight/postflight chain. Per-agentAction variation is expressed via `lifecycleConfig` fields (label, context bundle, result parser, branch source).
...
Five preflight scripts are loaded together by every PR-branch agentAction: `loadTaskState`, `loadConventions`, `loadPriorArt`, `loadMemoryContext`, `loadCoverageRules`. Bundle as `context: "task"`. Other bundles: `context: "goal"` (load goal state), `context: "minimal"` (none).
...
- A reviewer can read [AGENTS.md](../AGENTS.md) and predict, for any future agentAction, which lifecycle (if any) it should target.
...
### Phase 2 — Build `lifecycle: "pr-branch"` end-to-end on one agentAction
...
2. Add `src/lifecycles/index.ts` and `src/lifecycles/prBranch.ts` exporting a function `expand(profile) → profile` that:
   ...
   - For `lifecycle: "pr-branch"`: prepends the canonical preflight (sync → label → branch-source → context-bundle → composePrompt) and appends the canonical postflight (parseAgentResult → verify → commitAndPush → ensurePr → mirrorStateToPr → postIssueComment) to the profile's existing `preflight`/`postflight` arrays. Profile-supplied scripts run **after** lifecycle preflight and **before** lifecycle postflight so agentAction-specific steps still slot in.
3. Wire `expand` into `loadProfile` (or wherever profile.json is parsed today) so the executor never sees an unexpanded profile.
4. Migrate `src/agent-actions/fix/profile.json`:
...
- `git diff --stat src/agent-actions/fix/profile.json` shows the profile got smaller (the lifecycle macro replaced ~9 explicit entries).

**Risks**
- The canonical preflight order in `pr-branch` doesn't match one of the other 4 agentActions (`revert`, `resolve`, `fix-ci`, `init`). Don't worry about it yet — phase 3 will deal with mismatches by either adding `lifecycleConfig` flags or rejecting the agentAction from this lifecycle. If we discover the mismatches are deep, **stop and rethink before phase 3**.
...
2. Identify the agentAction-specific bits (e.g. `stageMergeConflicts` for `resolve`, `initFlow` for `init`).
3. Decide whether each bit is:
   - A standard step that should be in the lifecycle (add it, with a `lifecycleConfig` flag to opt in/out per agentAction).
   - Genuinely agentAction-specific (leave it in the profile's `scripts.preflight` / `scripts.postflight` — these run inside the lifecycle bookends).
...
- `resolve` has `stageMergeConflicts` — keep as an agentAction-specific preflight step inside the lifecycle.
...
**Risks**
- Cross-agentAction parity bugs: e.g., `revertFlow` did something subtly different than `fixFlow` for state mirroring. Mitigate with integration tests per agentAction before the migration, then re-run after.
- If two agentActions resist a shared lifecycle, that is a **signal** — either split into two lifecycle names, or accept that one of them stays bespoke and document why in `AGENTS.md`.
...
After phases 2–4, ~20 solo scripts should remain. These are genuinely agentAction-specific things that don't fit any lifecycle (e.g., `parseReproOutput`, `resolvePreviewUrl`, `warmupMcp`, `diagMcp`).

**Code changes**

1. Create the layout `src/scripts/shared/` and `src/scripts/agentAction/<name>/` (or any equivalent split that keeps registration in one place).
2. Move each remaining solo script from `src/scripts/` into `src/scripts/agentAction/<owning-name>/`. Registration stays in [src/scripts/index.ts](../src/scripts/index.ts) (or a delegating sub-registry) so profile references don't break.
3. Add `scripts/check-script-modularity.ts` (or extend an existing CI check) that:
   - Parses every `src/agent-actions/*/profile.json`.
   - Builds the `script-name → [agentActions]` map.
   - For every script in `src/scripts/shared/`, asserts `len(agentActions) ≥ 2`.
   - For every script in `src/scripts/agentAction/<name>/`, asserts it is referenced only by `<name>`'s profile.
...
- New directory split.
- New CI check.
- [CLAUDE.md](../CLAUDE.md) invariant 2 rewritten to describe the new layout (`shared/` versus `agentAction/<name>/`) and the mechanical check.
...
Strict dependency: phase 2 must ship and bake on `main` for at least one release before phase 3 starts. Reason: the lifecycle expansion lives in profile-loading code paths every agentAction shares — a regression there breaks everything at once, so we want a small blast radius first.
...
3. Should the CI check from phase 5 also enforce "no agentAction-local TS files outside `src/scripts/agentAction/<name>/`" (i.e., the existing invariant 2)? **Lean: yes**, since the check is already walking the tree.

## Out of scope (future work)

- Generalising hooks/skills/commands the same way (agentActions ship plugin parts today via `buildSyntheticPlugin`; that mechanism is fine and not part of this refactor).
...
- Re-evaluating the 30-agentAction count itself. Whether some agentActions should merge is a separate question — this refactor assumes today's agentAction set as given.
