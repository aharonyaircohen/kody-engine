...
6. **"Kody Capability" means `.kody/capabilities/<slug>.md`** — a markdown file in the consumer repo describing the capability's intent. Ticked by the existing `capability-scheduler` watch on every `kody.yml` wake. Contract: [src/executables/capability-tick/prompt.md](src/executables/capability-tick/prompt.md). When the user asks for "a capability" (the new domain noun for what was historically a "job"), this is the default shape — no YAML, no engine code, no publish. The legacy `job-*` executable family was renamed to `capability-*` in Phase 1 of the rename (issue #38); the `Job` runtime envelope in `src/job.ts` and the `kody-job-next-state` fence label are separate concerns and stay.
