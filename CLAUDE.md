...
- **Entry & dispatch** — [bin/kody.ts](bin/kody.ts) → [src/entry.ts](src/entry.ts). The only hardcoded verbs are `ci`, `help`, `version`, and the long-running servers (`serve`, `pool-serve`, `runner-serve`, `brain-serve`, `brain-proxy`, `mcp-http-server`). Everything else is a capability call resolved by [src/registry.ts](src/registry.ts) in order: consumer `.kody/capabilities/` → company store → engine built-ins under [src/implementations/](src/implementations/) + [src/capabilities/](src/capabilities/). [src/dispatch.ts](src/dispatch.ts) picks a capability from the GHA event when invoked as `kody ci`.
...
Adding a new command = a new capability folder (consumer `.kody/capabilities/<slug>/` or `kody-store`; engine built-ins under `src/implementations/` are the internal stdlib only) + register any new shared scripts. Issue-triggered commands need no dispatch edits — the PR switch in [src/dispatch.ts](src/dispatch.ts) does (no generic fallthrough there), and names overlapping via `\b…\b` word boundaries (e.g. `ui-review` vs `review`) must be ordered by specificity.
