...
pnpm build               # tsup bundle + node scripts/copy-assets.cjs (copies src/implementations, src/capabilities, src/plugins → dist/)
...
Two-layer design: **generic executor** + **declarative capability profile** + **script catalog**.
...
- **Entry & dispatch** — [bin/kody.ts](bin/kody.ts) → [src/entry.ts](src/entry.ts). The only hardcoded verbs are `ci`, `help`, `version`, and the long-running servers (`serve`, `pool-serve`, `runner-serve`, `brain-serve`, `brain-proxy`, `mcp-http-server`). Everything else is a capability call resolved by [src/registry.ts](src/registry.ts) in order: consumer `backend capability definitions` → company store → engine built-ins under [src/implementations/](src/implementations/) + [src/capabilities/](src/capabilities/). [src/dispatch.ts](src/dispatch.ts) picks a capability from the GHA event when invoked as `kody ci`.
- **Capability** — a `<slug>/` dir with `profile.json` (declaration) and `capability.md`/`prompt.md` (prose / agent instructions). The engine bundles only the internal stdlib fallback (`run` plus creator profiles); shared capabilities live in `kody-store`. No TypeScript inside capability directories. The profile declares CLI inputs, Claude Agent SDK config (tools, model, hooks, skills), `cliTools`, and the ordered preflight/postflight script list; lifecycles ([src/lifecycles/](src/lifecycles/)) inject the shared flow around them.
- **Scripts** ([src/scripts/](src/scripts/)) — TypeScript, registered in [src/scripts/index.ts](src/scripts/index.ts). **Only cross-cutting utilities used by multiple capabilities** (`commitAndPush`, `composePrompt`, `verify`, `ensurePr`, `postIssueComment`). The agent never commits — `commitAndPush` does.
...
2. **Capability directories contain only `profile.json`, `prompt.md`/`capability.md`, and optional plugin-part subdirs (`skills/`, `commands/`, `agents/`, `hooks/`).** TypeScript lives exclusively in `src/scripts/`, and only for logic shared across multiple capabilities. Plugin-part subdirs ship Claude Agent SDK assets (markdown / JSON, no TS) specific to that one capability; `buildSyntheticPlugin` resolves them from the capability dir first, then falls back to the central catalog under `src/plugins/`. **Design smell**: if a piece of logic is specific to one capability but too complex for its prompt/profile, either simplify it or promote it into a cross-cutting utility in `src/scripts/`. Never the middle ground ("just this once I'll put capability-specific TS somewhere").
...
6. **"Kody Capability" means `backend capability definition`** — a folder in the consumer repo (or company store) with `profile.json` metadata and `capability.md` prose describing intent. Ticked by `capability-scheduler` on every `kody.yml` wake; `capability-tick` runs one tick. When the user asks for "a capability" (the domain noun for what was historically a "job"), this is the default shape — no YAML, no engine code, no publish. The `Job` runtime envelope in `src/job.ts` and the `kody-job-next-state` fence label are separate concerns and stay.
7. **Scheduled work runs through schedulers, not extra workflows** — `capability-scheduler` / `goal-scheduler` fan out to due capabilities/goals on each `kody.yml` wake. Scheduler-driven observation capabilities report via `postIssueComment`, never `commitAndPush`. See the vocabulary table in [AGENTS.md](AGENTS.md) — don't call these "jobs" or "watches".

Adding a new command = a new capability folder (consumer `backend capability definition` or `kody-store`; engine built-ins under `src/implementations/` are the internal stdlib only) + register any new shared scripts. Issue-triggered commands need no dispatch edits — the PR switch in [src/dispatch.ts](src/dispatch.ts) does (no generic fallthrough there), and names overlapping via `\b…\b` word boundaries (e.g. `ui-review` vs `review`) must be ordered by specificity.
